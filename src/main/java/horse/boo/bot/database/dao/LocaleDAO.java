...
public class LocaleDAO {
    ...
    public List<Locale> index() {
        return jdbcTemplate.query("SELECT * FROM lyra_locale",
                new BeanPropertyRowMapper<>(Locale.class));
    }

    public Locale show(long id) {
        return jdbcTemplate.query("SELECT * FROM lyra_locale WHERE id=?",
                        new BeanPropertyRowMapper<>(Locale.class),
                        new Object[]{id})
    ...
    public void save(@NotNull Locale locale) {
        jdbcTemplate.update("INSERT INTO lyra_locale VALUES(default, ?, ?, ?, ?, ?)",
    ...
    public void update(long id, @NotNull Locale updatedLocale) {
        jdbcTemplate.update("UPDATE lyra_locale SET locale_name=?, locale_en=?, locale_ru=?, locale_ua=?, locale_cn=? WHERE id=?",
    ...
    public void delete(long id) {
        jdbcTemplate.update("DELETE FROM lyra_locale WHERE id=?",
                id);
    }
    ...
    public Locale getLocaleByName(String locale_name) {
        return jdbcTemplate.query("SELECT * FROM lyra_locale WHERE locale_name=?",
                        new BeanPropertyRowMapper<>(Locale.class),
